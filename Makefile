...
help:
	...
	@echo "    make token-stats   Show token usage stats"
...
verify:
	...
	@$(PYTHON) $(SCRIPTS_DIR)/token_tracker.py reset 2>/dev/null || echo "  Failed!"
	@echo ""
...
.PHONY: maestro
maestro:
	@$(PYTHON) scripts/maestro.py $(ARGS)

status:
	@$(PYTHON) scripts/maestro.py status

agents:
	@$(PYTHON) scripts/maestro.py agents

skills:
	@$(PYTHON) scripts/maestro.py skills
